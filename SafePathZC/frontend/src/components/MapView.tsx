  ...
  export const MapView = ({ onModalOpen }: MapViewProps) => {
    ...
    const [selectedRoute, setSelectedRoute] = useState<'safe' | 'manageable' | 'prone' | null>(null);
    ...
    const searchLocations = async (query: string): Promise<LocationSuggestion[]> => {
      ...
      try {
        ...
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query + ' Zamboanga City Mindanao Philippines')}&limit=3&addressdetails=1&bounded=1&viewbox=122.0,6.8,122.2,7.0`
        );
    ...
    const getOSRMRoute = async (start: LatLng, end: LatLng): Promise<LatLng[]> => {
    try {
      const response = await fetch(
  ...
  const getOSRMAlternativeRoutes = async (start: LatLng, end: LatLng): Promise<LatLng[][]> => {
    try {
      const response = await fetch(
  ...
  const getOSRMRouteWithWaypoint = async (start: LatLng, waypoint: LatLng, end: LatLng): Promise<LatLng[]> => {
    try {
      const response = await fetch(
    ...
    const calculateFloodRisk = (elevation: number, lat: number, lng: number): 'safe' | 'manageable' | 'prone' => {
      ...
      const cityCenter = { lat: 6.9214, lng: 122.0790 };
  ...
  const assessRouteFloodRisk = async (waypoints: LatLng[]): Promise<{
    ...
    samplePoints.forEach((point, index) => {
      const distanceFromCenter = Math.sqrt(
        Math.pow(point.lat - cityCenter.lat, 2) + Math.pow(point.lng - cityCenter.lng, 2)
      ) * 111;
    ...
    const averageElevation = validElevationPoints > 0 ? elevationSum / validElevationPoints : 0;
    
    // Enhanced risk assessment with real elevation data
    if (averageRisk >= 2.7 || lowElevationRatio > 0.7 || coastalRatio > 0.5 || averageElevation < 8) {
      return {
        risk: 'prone',
        level: 'High Risk',
        description: `Route passes through low-lying areas (avg ${averageElevation.toFixed(0)}m elevation). High flood risk during heavy rains.`,
        color: '#e74c3c'
      };
    } else if (averageRisk >= 1.8 || lowElevationRatio > 0.4 || coastalRatio > 0.3 || averageElevation < 20) {
      return {
        risk: 'manageable',
        level: 'Medium Risk',
        description: `Moderate elevation route (avg ${averageElevation.toFixed(0)}m elevation). Some flood risk during heavy rains.`,
        color: '#f39c12'
      };
    } else {
      return {
        risk: 'safe',
        level: 'Low Risk',
        description: `Higher elevation route (avg ${averageElevation.toFixed(0)}m elevation). Safer during floods but may be longer.`,
        color: '#27ae60'
      };
    }
  };
  ...
  const validateAndFixRoute = (route: LatLng[], start: LatLng, end: LatLng): LatLng[] => {
    ...
    const startsCorrectly = Math.abs(route[0].lat - start.lat) < threshold && 
                          Math.abs(route[0].lng - start.lng) < threshold;
    const endsCorrectly = Math.abs(route[route.length - 1].lat - end.lat) < threshold && 
  ...
  // NEW FUNCTION - Add this
  const areRoutesSimilar = (route1: LatLng[], route2: LatLng[], threshold: number): boolean => {
    if (route1.length === 0 || route2.length === 0) return false;
    
    const startDiff = Math.abs(route1[0].lat - route2[0].lat) + Math.abs(route1[0].lng - route2[0].lng);
  ...
  const generateFloodRoutes = async (start: LatLng, end: LatLng): Promise<RouteDetails> => {
    try {
  ...
  try {
    ...
    if (!manageableRouteData || manageableRouteData.length === 0) {
      ...
      manageableRouteData = alternatives[0] || createDirectRoute(start, end);
    }
  } catch (error) {
  ...
  const routePromises = [
    getOSRMRouteWithWaypoint(start, safeWaypoint, end).catch(async () => {
      ...
      const altSafeWaypoint = {
        lat: midLat + (directDistance * 0.08 * Math.cos(bearing + Math.PI/3)),
        lng: midLng + (directDistance * 0.08 * Math.sin(bearing + Math.PI/3))
      };
      return await getOSRMRouteWithWaypoint(start, altSafeWaypoint, end).catch(() => createDirectRoute(start, end));
    }),
    ...
    getOSRMRouteWithWaypoint(start, proneWaypoint, end).catch(async () => {
      ...
      return await getOSRMRouteWithWaypoint(start, altProneWaypoint, end).catch(() => createDirectRoute(start, end));
    })
  ];

  const [safeRouteData, finalManageableRoute, proneRouteData] = await Promise.all(routePromises);
  ...
  routes.push(finalSafeRoute, finalManagedRoute, finalProneRoute);
      ...
      const [safeAssessment, manageableAssessment, proneAssessment] = await Promise.all([
      ...
      const safeRoute = {
        ...
        riskLevel: 'Low Risk',
        description: `Higher elevation route (${safeAssessment.description.includes('avg') ? safeAssessment.description.match(/avg (\d+)m/)?.[1] || 'varied' : 'varied'} elevation). Safest during floods.`,
        color: '#27ae60'
      };
      ...
      return {
        safeRoute: { ...fallbackRoute, floodRisk: 'safe', color: '#27ae60', riskLevel: 'Low Risk' },
        manageableRoute: fallbackRoute,
        proneRoute: { ...fallbackRoute, floodRisk: 'prone', color: '#e74c3c', riskLevel: 'High Risk' },
        startName: selectedStartLocation?.display_name || 'Start Point',
        endName: selectedEndLocation?.display_name || 'End Point'
      };
    }
  ...
  const handleFindRoute = async () => {
    if (selectedStartLocation && selectedEndLocation) {
      ...
      try {
        ...
        ).bindTooltip('Safe Route', { permanent: true }).addTo(mapRef.current!);
        ...
        ).bindTooltip('Manageable Route', { permanent: true }).addTo(mapRef.current!);
        ...
        ).bindTooltip('Flood-Prone Route', { permanent: true }).addTo(mapRef.current!);
    ...
    useEffect(() => {
      ...
      const CollapsibleMenuBtn = L.Control.extend({
        ...
        onAdd: function () {
          const container = L.DomUtil.create('div', 'leaflet-bar leaflet-control collapsible-control');
          ...
          // Event handlers
          toggleBtn.onclick = (e) => { e.stopPropagation(); toggleMenu(); };
        ...
        <div style={{ marginBottom: '15px', display: 'flex', gap: '10px', alignItems: 'center' }}>
          {routeMode && routeDetails && (
            <div style={{
              ...
              <p style={{ margin: 0 }}>Flood-aware routes from {routeDetails.startName} to {routeDetails.endName}</p>
            </div>
          )}
    ...
    <div style={{
      ...
      <h4 style={{ margin: '0 0 8px 0', fontSize: '14px' }}>Route Options</h4>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ width: '20px', height: '3px', background: '#e74c3c', borderStyle: 'dashed' }}></div>
        ...
        {showRouteModal && routeDetails && (
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div 
                style={{
                  border: selectedRoute === 'safe' ? '3px solid #27ae60' : '2px solid #ddd',
                  borderRadius: '8px',
                  padding: '15px',
                  cursor: 'pointer',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{routeDetails.safeRoute.distance}</div>
                    <div style={{ fontSize: '0.9rem', color: '#666' }}>{routeDetails.safeRoute.time}</div>
                  </div>
                </div>
              ...
              <div 
                style={{
                  border: selectedRoute === 'manageable' ? '3px solid #f39c12' : '2px solid #ddd',
                  borderRadius: '8px',
                  padding: '15px',
                  cursor: 'pointer',
                  background: selectedRoute === 'manageable' ? '#fffbf0' : 'white',
                  transition: 'all 0.3s ease'
                }}
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <h3 style={{ margin: 0, color: '#f39c12' }}>⚠️ Manageable Route</h3>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{routeDetails.manageableRoute.distance}</div>
                    <div style={{ fontSize: '0.9rem', color: '#666' }}>{routeDetails.manageableRoute.time}</div>
                  </div>
                </div>
                <div style={{ marginBottom: '10px' }}>
                  <span style={{ 
              ...
              <div 
                style={{
                  border: selectedRoute === 'prone' ? '3px solid #e74c3c' : '2px solid #ddd',
                  borderRadius: '8px',
                  padding: '15px',
                  cursor: 'pointer',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <h3 style={{ margin: 0, color: '#e74c3c' }}>🚨 Flood-Prone Route</h3>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{routeDetails.proneRoute.distance}</div>
                    <div style={{ fontSize: '0.9rem', color: '#666' }}>{routeDetails.proneRoute.time}</div>
                  </div>
                </div>
                <div style={{ marginBottom: '10px' }}>
                  <span style={{ 
            ...
            {selectedRoute && (
              <div style={{
                ...
                <h4 style={{ margin: '0 0 10px 0', color: '#2c3e50' }}>
                  📍 Selected Route: {selectedRoute.charAt(0).toUpperCase() + selectedRoute.slice(1)} Route
                </h4>
                <p style={{ margin: '0', fontSize: '0.9rem', color: '#666' }}>
                  Route displayed on map. The {selectedRoute} route is now highlighted in {
                    selectedRoute === 'safe' ? 'green' : 
                    selectedRoute === 'manageable' ? 'orange' : 'red'
                  }.
                </p>
              </div>
            )}
            ...
            <div style={{
              ...
              <h4 style={{ margin: '0 0 10px 0', color: '#856404' }}>💡 Flood Safety Tips</h4>
