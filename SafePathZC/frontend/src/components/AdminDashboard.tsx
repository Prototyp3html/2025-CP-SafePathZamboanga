...
import { Check, AlertTriangle, Eye, EyeOff, Filter, Search, Users, MapPin, Clock, Flag } from 'lucide-react';
...
interface Report {
  ...
  category: 'flood' | 'road_closure' | 'accident' | 'emergency' | 'infrastructure' | 'other';
...
export const AdminDashboard: React.FC = () => {
  ...
  const updateReportStatus = async (reportId: string, status: 'approved' | 'rejected', notes?: string) => {
    try {
      const token = localStorage.getItem('admin_token');
      if (!token) return;

      const response = await fetch(`${BACKEND_URL}/admin/reports/${reportId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const toggleReportVisibility = async (reportId: string, isVisible: boolean) => {
    try {
      const token = localStorage.getItem('admin_token');
      if (!token) return;

      const response = await fetch(`${BACKEND_URL}/admin/reports/${reportId}/visibility`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <button
                          onClick={() => toggleReportVisibility(report.id, !report.isVisible)}
                          className={`p-2 rounded-lg transition-colors ${
                            report.isVisible 
                              ? 'bg-green-100 text-green-600 hover:bg-green-200' 
                              : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                          }`}
                          title={report.isVisible ? 'Hide from public' : 'Show to public'}
                        >
                          {report.isVisible ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                        </button>
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900">{Array.isArray(reports) ? reports.length : 0}</p>
                </div>
              </div>
            </div>
