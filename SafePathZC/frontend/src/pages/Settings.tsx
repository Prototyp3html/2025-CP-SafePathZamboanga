...
const Settings = () => {
  ...
  const { preferences, updatePreference, savePreferences, isLoading } = usePreferences();
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      if (parsedUser.twoFactorEnabled !== undefined) {
        setSecurity(prev => ({ ...prev, twoFactorEnabled: parsedUser.twoFactorEnabled }));
      }
    } else if (savedProfilePicture) {
      setProfilePicture(savedProfilePicture);
    }
  ...
  const handleSaveAllSettings = async () => {
    try {
      ...
      toast({
        ...
        description: "Your settings and preferences have been updated successfully.",
      });
    } catch (error) {
      toast({
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: data.detail || "Failed to update password. Please check your current password.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleEnableTwoFactor = async () => {
    ...
    try {
      ...
      if (security.twoFactorEnabled) {
        ...
        // Enable 2FA
        const phoneNumber = prompt("Enter your phone number for SMS verification:");
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: data.detail || "Failed to delete account. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <>
      ...
      {isAdmin ? (
        ...
        <div className="mt-8 p-4 bg-gray-50 rounded-lg border">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-gray-900">Save All Settings</h3>
              <p className="text-sm text-gray-600">Save all your changes including preferences, notifications, and profile.</p>
            </div>
            <Button 
