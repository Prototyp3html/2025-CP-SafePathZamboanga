...
const Alerts = () => {
  ...
  const [activeWarnings, setActiveWarnings] = useState<(CommunityReport | WeatherAlert)[]>([]);
    ...
    try {
      ...
      if (current.precip_mm > 20) {
        ...
        setEmergencyAlert({
          type: 'flood',
          severity: 'high',
          message: `Heavy flooding reported in multiple areas. ${current.precip_mm.toFixed(1)}mm rainfall detected. Avoid travel unless absolutely necessary.`,
        ...
        generatedAlerts.push({
          ...
          type: 'flood',
          severity: 'high',
          title: `Severe Flooding Warning - ${current.precip_mm.toFixed(1)}mm Rainfall`,
          description: `Heavy flooding expected in low-lying areas. Current rainfall: ${current.precip_mm.toFixed(1)}mm/hr. Water levels may rise rapidly.`,
        ...
        generatedAlerts.push({
          ...
          type: 'flood',
          severity: 'moderate',
          title: `Moderate Flooding Risk - ${current.precip_mm.toFixed(1)}mm Rainfall`,
      ...
      if (current.wind_kph > 60) {
        ...
        setEmergencyAlert({
          type: 'wind',
          severity: 'high',
          message: `Very strong winds detected (${current.wind_kph.toFixed(0)}kph). Secure loose objects and avoid exposed areas.`,
        ...
        if (reportsResponse.ok) {
          ...
          communityReports.forEach((report: any) => {
            generatedAlerts.push({
              ...
              title: `${report.type.charAt(0).toUpperCase() + report.type.slice(1)} - ${report.location}`,
      ...
      try {
        ...
        if (pagasaResponse.ok) {
          ...
          if (pagasaData.success && pagasaData.bulletins && pagasaData.bulletins.length > 0) {
            ...
            pagasaData.bulletins.forEach((bulletin: any, index: number) => {
              updates.push({
                ...
                description: bulletin.content || bulletin.description || 'No details available',
                timestamp: bulletin.timestamp || 'Recently updated',
                postedAt: new Date().toISOString(), // For sorting
                source: 'PAGASA Official',
                bulletinType: bulletin.type === 'typhoon' ? '🌪️ Typhoon Bulletin' : 
                             bulletin.type === 'rainfall' ? '🌧️ Rainfall Advisory' :
                             '⚠️ Weather Advisory',
                impact: bulletin.type === 'typhoon' ? 'Possible severe weather conditions' :
                        bulletin.type === 'rainfall' ? 'Flooding possible in low-lying areas' :
            ...
            console.log(`✅ Loaded ${pagasaData.bulletins.length} PAGASA bulletins`);
          } else {
      ...
      updates.push({
        ...
        description: `${current.condition.text} with temperature at ${current.temp_c}°C. ${
        ...
        impact: current.precip_mm > 10 ? 'High flood risk in low-lying areas' : 
                current.precip_mm > 5 ? 'Moderate flood risk' : 'Low flood risk',
      ...
      if (forecast) {
        ...
        const nextHours = hourlyData.slice(new Date().getHours(), new Date().getHours() + 6);
        const maxRain = Math.max(...nextHours.map((h: any) => h.precip_mm));
        const avgWind = nextHours.reduce((sum: number, h: any) => sum + h.wind_kph, 0) / nextHours.length;
        ...
        if (maxRain > 5 || avgWind > 30) {
          updates.push({
            ...
            description: `Expected conditions: Maximum rainfall of ${maxRain.toFixed(1)}mm/hr, average wind speed ${avgWind.toFixed(0)}kph. ${
              maxRain > 15 ? 'Heavy rain expected.' : maxRain > 8 ? 'Moderate rain expected.' : 'Light rain possible.'
            }`,
            timestamp: 'Updated now',
            postedAt: new Date().toISOString(),
            source: 'WeatherAPI Forecast',
            impact: maxRain > 15 ? 'Possible flash flooding in vulnerable areas' : 'Minor inconveniences expected',
            recommendations: [
              maxRain > 10 ? 'Delay non-essential travel' : 'Allow extra travel time',
      ...
      console.log(`✅ Loaded ${generatedAlerts.length} active warnings and ${updates.length} weather updates`);
      ...
      setWeatherUpdates([{
        ...
        description: 'Unable to fetch current weather conditions. Please check your internet connection and try again.',
        timestamp: 'Error',
        source: 'System',
        impact: 'Weather monitoring temporarily unavailable',
        recommendations: ['Refresh the page', 'Check internet connection', 'Try again later'],
        isBookmarked: false
      }]);
      setLoading(false);
    }
  ...
  const handleShareAlert = (alert: any) => {
    ...
    if (navigator.share) {
      ...
      toast({
        title: "Alert Copied",
        description: "Alert details copied to clipboard. You can now paste and share it.",
      });
    }
  ...
  const handleEmergencyContact = () => {
    toast({
      ...
      description: "Emergency: 911 | Disaster Risk Reduction: (062) 991-2999 | Fire: 160 | Police: 117",
      duration: 10000,
    });
  };
  ...
  const getSeverityIcon = (type: string) => {
    switch (type) {
      ...
      case 'storm': return 'fas fa-cloud-bolt';
      case 'wind': return 'fas fa-wind';
      case 'traffic': return 'fas fa-car';
      case 'roadblock': return 'fas fa-road-barrier';
      case 'damage': return 'fas fa-road-spikes';
      ...
      <main className="pt-20 container mx-auto px-4 py-8">
        ...
        {emergencyAlert && (
          <Alert className={`mb-8 border-red-500 ${
            ...
            <i className={`${getSeverityIcon(emergencyAlert.type)} text-red-600`}></i>
        ...
        {!emergencyAlert && !loading && activeWarnings.length === 0 && (
          <Alert className="mb-8 border-green-500 bg-green-50 animate-fade-in">
            ...
            <AlertDescription className="text-green-700">
              No active weather warnings or emergencies at this time. Conditions are safe for travel.
            </AlertDescription>
          </Alert>
        )}
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="text-lg text-gray-600">Loading alerts data...</span>
          </div>
        )}
              ...
              {activeWarnings.length === 0 ? (
                <Card className="text-center py-12">
                  <CardContent>
                    ...
                    <h3 className="text-xl font-semibold text-gray-800 mb-2">No Active Warnings</h3>
                    <p className="text-gray-600">
                      There are currently no active weather warnings or community reports. Safe travels!
                    </p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid gap-6">
                  {activeWarnings.map((warning) => (
                    <Card key={warning.id} className={`border-l-4 hover:shadow-lg transition-shadow ${
                      warning.severity === 'high' ? 'border-l-red-500' :
                      warning.severity === 'moderate' ? 'border-l-orange-500' : 'border-l-yellow-500'
                    } animate-fade-in`}>
                      <CardHeader>
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    ...
                    Latest bulletins and weather news • {weatherUpdates.length} update{weatherUpdates.length !== 1 ? 's' : ''}
                  </p>
                </div>
                <div className="flex space-x-2">
                  <Button
                ...
                {weatherUpdates.length === 0 ? (
                  <Card className="bg-gray-50">
                    <CardContent className="p-6 text-center">
                      ...
                      <p className="text-gray-600">No weather updates available at this time.</p>
                      <p className="text-sm text-gray-500 mt-2">Check back later for the latest bulletins.</p>
                    </CardContent>
                  </Card>
                ) : (
                  weatherUpdates.map((update, index) => (
                    <Card 
                      ...
                      className={`hover:shadow-lg transition-all duration-300 animate-fade-in ${
                        update.isPagasa ? 'border-l-4 border-l-orange-500 bg-gradient-to-r from-orange-50 to-white' : 
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-start space-x-3 flex-1">
                            ...
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white text-xl font-bold ${
                              update.isPagasa ? 'bg-gradient-to-br from-orange-500 to-orange-600' : 
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-bold text-gray-800">
                                  {update.isPagasa ? 'PAGASA' : 'SafePath Weather'}
                                </span>
                                {update.isPagasa && (
                              ...
                              <CardDescription className="text-xs flex items-center gap-2">
                                ...
                                {update.source && (
                                  <>
                                    <span>•</span>
                                    <span className="text-gray-500">{update.source}</span>
                                  </>
                                )}
                              </CardDescription>
                            </div>
                          ...
                          <Button
                            ...
                            className={update.isBookmarked ? 'text-yellow-500 hover:text-yellow-600' : 'text-gray-400 hover:text-gray-600'}
                          >
                        ...
                        {update.impact && (
                          <div className="bg-blue-50 border-l-4 border-blue-400 rounded-r-lg p-3 mb-4">
                            <div className="flex items-start">
                              ...
                              <div>
                                <h4 className="font-semibold text-blue-900 text-sm">Expected Impact</h4>
                                <p className="text-blue-800 text-sm mt-1">{update.impact}</p>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        {update.recommendations && update.recommendations.length > 0 && (
                          <div className="bg-amber-50 border-l-4 border-amber-400 rounded-r-lg p-3 mb-4">
                            ...
                            <ul className="text-sm text-amber-800 space-y-1.5">
                              {update.recommendations.map((rec: string, index: number) => (
