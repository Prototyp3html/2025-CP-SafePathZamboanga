#!/usr/bin/env python3
"""
Populate localhost with sample flood data for demonstration
Same scenarios as production:
1. Dec 14: Heavy rain (5mm) with flooding
2. Dec 17: Moderate rain (2mm) for 3 hours
...
# Import models
from models import FloodEventLog, FloodHotspot, SessionLocal
...
def populate_localhost_sample_data():
    """Populate localhost with sample flood events"""
    
    db: Session = SessionLocal()
    
    try:
        print("[*] Populating localhost with sample flood data...")
        ...
        db.commit()
        print(f"\n[+] Created {events_created} sample flood events on localhost")
        
        # Verify data
        from sqlalchemy import func
        total_events = db.query(FloodEventLog).count()
        event_types = db.query(FloodEventLog.event_type, func.count(FloodEventLog.id)).group_by(FloodEventLog.event_type).all()
        ...
        print("\n[!] IMPORTANT: Run hotspot recalculation on localhost!")
        print("    Click 'Update Flood Data Now' or run: python trigger_hotspot_recalc.py")
...
if __name__ == "__main__":
    print("=" * 60)
    print("POPULATE LOCALHOST WITH SAMPLE FLOOD DATA")
    print("=" * 60)
    populate_localhost_sample_data()
    print("=" * 60)
